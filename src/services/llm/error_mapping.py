...
def map_error(exc: Exception, provider: Optional[str] = None) -> LLMError:
    ...
    # Project providers already raise these unified exceptions. Preserve their
    # status/message instead of wrapping them repeatedly.
    if isinstance(exc, LLMError):
        return exc

    if isinstance(exc, (asyncio.TimeoutError, TimeoutError)):
        return LLMTimeoutError("模型服务响应超时", provider=provider)
