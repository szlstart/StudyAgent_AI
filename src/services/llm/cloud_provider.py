...
async def _openai_complete(
    ...
    **kwargs,
    ...
    # Detect multimodal content (image_url in messages array).
    # lightrag cannot handle image content — skip it and go directly to aiohttp.
    def _has_image_content(msgs) -> bool:
        if not msgs:
            return False
        for msg in msgs:
            c = msg.get("content", "")
    ...
    _messages_kwarg = kwargs.get("messages")
    _is_multimodal = _has_image_content(_messages_kwarg)

    if not _is_multimodal:
        try:
            # Try using lightrag's openai_complete_if_cache first (has caching).
            # Filter out keys that lightrag doesn't accept.
            _lightrag_excluded = {"messages", "response_format"}
            lightrag_kwargs = {
                "system_prompt": system_prompt,
                "history_messages": [],  # Required by lightrag to build messages array
                "api_key": api_key,
                "base_url": base_url,
                **{k: v for k, v in kwargs.items() if k not in _lightrag_excluded},
            }
            if api_version:
            ...
            pass  # Fall through to direct aiohttp call
    ...
    if not content and base_url:
        ...
        # Use pre-built messages array if provided (e.g., for multimodal / image calls).
        # Otherwise build a standard [system, user] pair from prompts.
