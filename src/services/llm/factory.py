...
def _is_transient_gateway_model_error(error: BaseException, base_url: str | None) -> bool:
    """Retry Apinebula's intermittent model-channel 404, not ordinary 404s."""
    if not isinstance(error, LLMAPIError) or error.status_code != 404:
        return False
    try:
        host = (urlparse(base_url or "").hostname or "").lower()
    except ValueError:
        return False
    if host != "apinebula.ai":
        return False
...
async def complete(
    ...
    **kwargs,
    ...
    def _is_retriable_llm_api_error(exc: BaseException) -> bool:
        ...
        if _is_transient_gateway_model_error(exc, base_url):
            return True
    ...
    @tenacity.retry(
        ...
        # Preserve the provider exception after retries are exhausted.  Wrapping it
        # in tenacity.RetryError hides the HTTP status and makes API layers unable
        # to distinguish an unavailable model from an unreadable image.
        reraise=True,
    )
