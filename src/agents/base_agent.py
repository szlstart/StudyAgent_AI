...
class BaseAgent(ABC):
    ...
    def __init__(
        ...
        # Allow per-model temperature override (e.g. kimi-k2.5 requires temperature=1)
        if temperature is not None:
            self._agent_params = dict(self._agent_params)   # defensive copy
            self._agent_params["temperature"] = temperature
    ...
    async def call_llm(
        ...
        try:
            response = await llm_complete(
                ...
                binding=self.binding,   # 传递 binding，确保特殊 provider 的 header 正确注入
